...
function arvutaKogusumma() {
    ...
    korv.forEach(korvArtikkel => {
        let hind = parseHind(korvArtikkel.hind); 
        let kogus = korvArtikkel.kogus; 
        summa += hind * kogus; 
    });
...
// Abifunktsioon, mis teisendab hinnad numbriteks
function parseHind(hindStr) {
    return parseFloat(hindStr.replace('€', '').trim()); 
}


function kuvaKogusumma(summa) {
    const summaElem = document.querySelector('.korv-summa');
    if (summaElem === null) {
        korviSisu.insertAdjacentHTML('afterend', `<h3 class="korv-summa">Kokku: ${summa.toFixed(2)} €</h3>`);
    } else {
        summaElem.innerText = `Kokku: ${summa.toFixed(2)} €`;
    }
}
...
form.addEventListener("submit", (e) => {
    ...
    if (eesnimi.value.trim() === "") {
        ...
        errors.push("Ei saa olla numbreid!");
    }
    ...
    if (epost.value.trim() === "" || !/\S+@\S+\.\S+/.test(epost.value)) { 
        errors.push("Sisesta kehtiv e-post!");
    }
    ...
    if (telefon.value.trim() === "") {
        ...
        errors.push("Liiga lühike telefoninumber!");
    }
    ...
    const tarneviisValitud = document.querySelector('input[name="tarne"]:checked');
    if (!tarneviisValitud) {
    ...
    if (messageField.value.length > 20) { 
        errors.push("Sõnum ei tohi ületada 20 sümbolit!");
    }
